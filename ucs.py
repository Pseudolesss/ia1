...
class PacmanAgent(Agent):
    ...
    def get_path(self, queue):
        ...
        Return:
        -------
         - A dictionary with pairs (Pacman pos, food pos)
        as keys and actions as values.

        Each key originates from a state that is part of the UCS path solution
        starting from the argument state.
        """
        i = 0  # Implicit value to avoid identical priority
        ...
        while True:
            ...
            # Consider the state linked to the first element
            state = entry[1][0]
